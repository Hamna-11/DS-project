...
PeopleManager::Room::Room(int i, std::string n, int cap) : id(i), name(n), capacity(cap) {}
...
int PeopleManager::getStudentCount() const { return students.size(); }
int PeopleManager::getFacultyCount() const { return faculties.size(); }
int PeopleManager::getRoomCount() const { return rooms.size(); }

void PeopleManager::listAllStudents() const {
    std::cout << "\n" << std::string(60, '=') << "\n ALL STUDENTS\n" << std::string(60, '=') << "\n\n";
    std::cout << std::left << std::setw(5) << "ID" << std::setw(25) << "Name" << "Enrolled Courses\n";
    std::cout << std::string(60, '-') << "\n";
    for (const auto& s : students) {
        std::cout << std::left << std::setw(5) << s.id << std::setw(25) << s.name << "[ ";
...
void PeopleManager::listAllFaculties() const {
    std::cout << "\n" << std::string(60, '=') << "\n ALL FACULTY\n" << std::string(60, '=') << "\n\n";
    std::cout << std::left << std::setw(5) << "ID" << std::setw(25) << "Name" << "Teaching Courses\n";
    std::cout << std::string(60, '-') << "\n";
    for (const auto& f : faculties) {
        std::cout << std::left << std::setw(5) << f.id << std::setw(25) << f.name << "[ ";
...
void PeopleManager::listAllRooms() const {
    std::cout << "\n" << std::string(60, '=') << "\n ALL ROOMS\n" << std::string(60, '=') << "\n\n";
    std::cout << std::left << std::setw(5) << "ID" << std::setw(20) << "Name" << "Capacity\n";
    std::cout << std::string(40, '-') << "\n";
    for (const auto& r : rooms) {
        std::cout << std::left << std::setw(5) << r.id << std::setw(20) << r.name << r.capacity << "\n";
    }
